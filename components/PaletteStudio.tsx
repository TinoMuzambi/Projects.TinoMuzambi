...
function hasReadableRoles(colors: Palette) {
	return (
		contrastRatio(colors.ink, colors.paper) >= 4.5 &&
		contrastRatio(colors.ink, colors.surface) >= 4.5 &&
		contrastRatio(colors.primary, colors.paper) >= 4.5
	)
}
...
export function PaletteStudio() {
	...
	useEffect(() => {
		...
		try {
			...
			if (saved) {
				...
				if (isPalette(parsed) && hasReadableRoles(parsed)) {
	...
	useEffect(() => {
		if (!hasLoaded) return
		if (!hasReadableRoles(colors)) return
		applyPalette(colors)
		try {
			localStorage.setItem(storageKey, JSON.stringify(colors))
		} catch {
			// The palette still applies for the current page when storage is unavailable.
		}
	}, [colors, hasLoaded])
	...
	const rolesAreReadable = hasReadableRoles(colors)
	...
	return (
		<>
			...
			{isOpen ? (
				<aside className="palette-studio" id="palette-studio" aria-labelledby="palette-title" ref={panelRef} role="dialog">
					<header>
						<div>
							<h2 id="palette-title">Colour palette</h2>
							<p>Try a preset or tune each role.</p>
						</div>
						<button type="button" onClick={closeAndRestoreFocus} aria-label="Close colour palette">
							Close
						</button>
					</header>
					...
					<div className="palette-presets" aria-label="Palette presets">
						{presets.map((preset) => (
							<button
								...
								type="button"
								key={preset.id}
								aria-pressed={activePalette === preset.id}
					...
					{!rolesAreReadable ? (
						<p className="palette-warning" role="status">
							This combination has not been applied because it reduces text contrast. Choose a preset or increase the difference between text and its surfaces.
						</p>
					) : null}
